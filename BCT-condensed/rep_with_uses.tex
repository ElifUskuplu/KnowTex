...
\begin{defn}
\uses{defn:hom-func, defn:isomorphism}
\label{defn:repr-func}
...
\begin{pf}\uses{lemma:adj-to-rep}
...
Let us first dualize Definition~\ref{defn:hom-func}.

\begin{defn}
\uses{defn:functor}
\label{defn:contra-hom-func}
Let $\cat{A}$ be a locally small category and $A \in \cat{A}$.  We define a
functor
\[
...
\begin{defn}\label{defn:contra-repr-func}
\uses{defn:contra-hom-func, defn:isomorphism}
...
\begin{thm}[Yoneda]
\uses{defn:contra-hom-func, defn:nat-in}
\label{thm:yoneda}
%
\index{Yoneda lemma}
%
...
\begin{lemma}
\uses{defn:full-faithful, defn:isomorphism}
...
\begin{pf}\uses{cor:yoneda-ff, lemma:ff-refl-isos}
