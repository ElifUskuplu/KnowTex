...
\begin{definition}[Abelian Group]\label{def:abelian}
...
\begin{example}\label{ex:symmetric-group}
...
\begin{lemma}\label{lem:coset-equality}
...
\begin{corollary}\label{cor:order-divides}
...
\begin{corollary}\label{cor:prime-cyclic}
...
\begin{corollary}[Fermat--Euler]\label{cor:fermat-euler}
...
\begin{proposition}\label{prop:normal-equiv}
...
\begin{example}\label{ex:normal-examples}
...
\begin{example}\label{ex:Z-mod-n}
...
\begin{definition}[Homomorphism]\label{def:homo}
...
\begin{corollary}\label{cor:euler-phi}
...
\begin{definition}[External Direct Product]\label{def:direct-product}
...
More precisely, if $|G| = n$, then $G$ embeds into $S_n$.
\end{theorem}
...
\begin{theorem}[Sylow Theorems]\label{thm:sylow-thm}
...
\begin{corollary}\label{cor:sylow-normal}
