...
\begin{proof}\leanok Consider the graph $A \subset G \times G'$ defined by
$$ A := \{ (x,f(x)): x \in G \}.$$
...
\begin{proof} \leanok The map $\phi \mapsto \phi(x)$ is a homomorphism from $\mathrm{Hom}(G,\Z/2\Z)$ to $\Z/2\Z$, and by Lemma \ref{gdual} the kernel has order equal to the order of $G / \{0,x\}$, which is $|G|/2$.  Then the preimage of $1$ must also be of order $|G|/2$.
\end{proof}
...
\begin{proof}\leanok By Theorem \ref{approx-hom-pfr}, there exists a homomorphism $\phi: G \to G'$ and a constant $c \in G'$ such that the set $A := \{ x \in G: f(x) = \phi(x) + c\}$ has cardinality at least $|G| / (2 ^ {172} * K ^ {146})$.  By Lemma \ref{gslice}, there exists a homomorphism $\phi': G \to \Z/2\Z$ such that
