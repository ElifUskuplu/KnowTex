...
\begin{lemma}[Entropy and relabeling]\label{relabeled-entropy}\lean{ProbabilityTheory.entropy_comp_of_injective, ProbabilityTheory.entropy_of_comp_eq_of_comp} \leanok
  \begin{itemize}
...
\begin{proof} \leanok Apply the ``averaging over conditioning'' argument to \Cref{cond-reduce}.
\end{proof}
...
\begin{proof}  \leanok Apply \Cref{submodularity} and \Cref{chain-rule}.
\end{proof}
...
\begin{proof} \leanok Immediate from \Cref{vanish-entropy} and \Cref{conditional-independent-def}.
\end{proof}
...
\begin{proof}\leanok Immediate from \Cref{conditional-vanish} and \Cref{conditional-mutual-alt}.
\end{proof}
