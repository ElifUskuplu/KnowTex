...
\begin{proof}\leanok Immediate from \Cref{copy-ent}.
\end{proof}
...
\begin{proposition}[$\tau$ has minimum]\label{tau-min}
  \lean{tau_minimizer_exists}\leanok
A pair $X_1, X_2$ of $\tau$-minimizers exist.
\end{proposition}

\begin{proof}\leanok By \Cref{tau-copy}, $\tau$ only depends on the probability distributions of $X_1, X_2$. This ranges over a compact space, and $\tau$ is continuous.  So $\tau$ has a minimum.
\end{proof}
...
\begin{proof}\leanok  Apply \Cref{distance-lower} to conditioned random variables and then average.
\end{proof}
...
\begin{proof} \leanok  Immediate from \Cref{cor-fibre}.
\end{proof}
...
\begin{proof}\leanok Immediate from \Cref{distance-lower}.
\end{proof}
...
\begin{proof}\leanok Immediate from \Cref{cond-distance-lower}.
\end{proof}
...
\begin{proof} \leanok  Immediate from \Cref{first-useful} (and recalling that $k$ is defined to be $d[X_1;X_2]$).
\end{proof}
...
\begin{proof}\leanok  Take a suitable linear combination of \Cref{first-fibre}, \Cref{first-dist-sum}, \Cref{first-cond}, and \Cref{first-upper}.
\end{proof}
...
\begin{proof}\leanok From \Cref{distance-lower} one has
\begin{align*}
...
\begin{proof}  \leanok  Let $X_1, X_2$ be the $\tau$-minimizer from \Cref{tau-min}.  From \Cref{de-prop}, $d[X_1;X_2]=0$.  From \Cref{lem:100pc}, $d[X_1;U_H] = d[X_2; U_H] = 0$.  Also from $\tau$-minimization we have $\tau[X_1;X_2] \leq \tau[X^0_2;X^0_1]$.  Using this and the Ruzsa triangle inequality we can conclude.
\end{proof}
