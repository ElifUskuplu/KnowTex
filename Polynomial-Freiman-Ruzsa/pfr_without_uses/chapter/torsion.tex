...
\begin{proof}\leanok  Apply \Cref{data-process-unc} to $X,Y$ conditioned to the event $Z=z$, multiply by ${\bf P}[Z=z]$, and sum using \Cref{conditional-mutual-def}.
\end{proof}
...
\begin{proof}\leanok  This is trivial for $n=0,1$, while the $n=2$ case is \Cref{kv}.  Now suppose inductively that $n > 2$, and the claim was already proven for $n-1$.  By a further application of \Cref{kv} one has
...
\begin{proof}\leanok  Write $W := \sum_{i=1}^m X_i$.  From \Cref{sumset-lower} we have
...
\begin{proof}\leanok From \Cref{kv} one has
...
\begin{lemma}[Nonnegativity]\label{multidist-nonneg}\lean{multiDist_nonneg}\leanok  For any such tuple, we have $D[X_{[m]}] \geq 0$.
\end{lemma}

\begin{proof}\leanok  From \Cref{sumset-lower} one has
$$ \bbH[\sum_{i =1}^m \tilde X_i] \geq \bbH[\tilde X_i]$$
for each $1 \leq i \leq m$.  Averaging over $i$, we obtain the claim.
\end{proof}

\begin{lemma}[Relabeling]\label{multidist-perm}\lean{multiDist_of_perm}\leanok If $\phi: \{1,\dots,m\} \to \{1,\dots,m\}$ is a bijection, then $D[X_{[m]}] = D[(X_{\phi(j)})_{1 \leq j \leq m}]$.
\end{lemma}

\begin{proof}\leanok Trivial.
\end{proof}

\begin{lemma}[Multidistance and Ruzsa distance, I]\label{multidist-ruzsa-I}\lean{multidist_ruzsa_I}\leanok
...
\begin{lemma}[Multidistance and Ruzsa distance, II]\label{multidist-ruzsa-II}\lean{multidist_ruzsa_II}\leanok
  Let $m \ge 2$, and let $X_{[m]}$ be a tuple of $G$-valued random variables. Then
...
\begin{lemma}[Multidistance and Ruzsa distance, III]\label{multidist-ruzsa-III}\lean{multidist_ruzsa_III}\leanok
...
\begin{lemma}[Multidistance and Ruzsa distance, IV]\label{multidist-ruzsa-IV}\lean{multidist_ruzsa_IV}\leanok
...
\begin{proposition}[Vanishing]\label{multi-zero}\lean{multidist_eq_zero}\leanok
If $D[X_{[m]}]=0$, then for each $1 \leq i \leq m$ there is a finite subgroup $H_i \leq G$ such that $d[X_i; U_{H_i}] = 0$.
\end{proposition}

\begin{proof}\leanok  From \Cref{multidist-ruzsa-II} and \Cref{ruzsa-nonneg} we have $d[X_j; X_j]=0$ for all $1 \leq j \leq m$.  The claim now follows from \Cref{lem:100pc}.
\end{proof}
...
\begin{definition}[$\tau$-functional]\label{tau-def-multi}\lean{multiTau}\leanok  If $(X_i)_{1 \leq i \leq m}$ is a tuple, we define its $\tau$-functional
...
\begin{proof}\leanok This is similar to the proof of \Cref{tau-min}.
\end{proof}
...
\begin{proof}\leanok  By \Cref{tau-min-multi} we have
...
\begin{definition}[Conditional multidistance]\label{cond-multidist-def}\lean{condMultiDist}
...
\begin{lemma}[Conditional multidistance nonnegative]\label{cond-multidist-nonneg}\lean{condMultiDist_nonneg}\leanok If $X_{[m]} = (X_i)_{1 \leq i \leq m}$ and $Y_{[m]} = (Y_i)_{1 \leq i \leq m}$ are tuples of random variables, then $D[ X_{[m]} | Y_{[m]} ] \geq 0$.
\end{lemma}

\begin{proof}\leanok Clear from \Cref{multidist-nonneg} and \Cref{cond-multidist-def}, except that some care may need to be taken to deal with the $y_i$ where $p_{Y_i}$ vanish.
\end{proof}
...
\begin{proof}\leanok  This follows from \Cref{cond-multidist-lower} and \Cref{multidist-perm}.
\end{proof}

\section{The multidistance chain rule}

\begin{lemma}[Multidistance chain rule]\label{multidist-chain-rule}\lean{multiDist_chainRule}\leanok  Let $\pi \colon G \to H$ be a homomorphism of abelian groups and let $X_{[m]}$ be a tuple of jointly independent $G$-valued random variables.  Then $D[X_{[m]}]$ is equal to
  \begin{equation}
...
\begin{proposition}[Mutual information bound]\label{prop:52}\lean{mutual_information_le_t_12, mutual_information_le_t_13, mutual_information_le_t_23}\leanok
  We have
  \[
    \bbI[Z_1 : Z_2\, |\, W],\
    \bbI[Z_2 : Z_3\, |\, W],\
...
\begin{lemma}[Entropy of $W$]\label{ent-w}\lean{entropy_of_W_le}\leanok  We have $\bbH[W] \leq (2m-1)k + \frac1m \sum_{i=1}^m \bbH[X_i]$.
\end{lemma}

\begin{proof}\leanok Without loss of generality, we may take $X_1,\dots,X_m$ to be independent. Write $S = \sum_{i=1}^m X_i$.
...
\begin{lemma}[Entropy of $Z_2$]\label{ent-z2}\lean{entropy_of_Z_two_le}\leanok  We have $\bbH[Z_2] \leq (8m^2-16m+1) k + \frac{1}{m} \sum_{i=1}^m \bbH[X_i]$.
\end{lemma}
...
\begin{proof}\leanok For each value $W=w$, apply \Cref{lem:get-better} (and \Cref{Zero-sum}) to
  \[
    T_1 = (Z_1 \,|\, W \mathop{=} w),\
    T_2 = (Z_2 \,|\, W \mathop{=} w),\
    T_3 = (Z_3 \,|\, W \mathop{=} w)
...
\begin{proof}\leanok  Repeat the proof of \Cref{pfr_aux}, but with \Cref{main-entropy} in place of \Cref{entropy-pfr}.  Because of the lack of $2$-torsion, one has to use the Ruzsa triangle inequality to bound $d[U,U]$ by $2d[U,-U]$, costing an additional factor of $2$ in the estimates.
\end{proof}
...
\begin{theorem}[PFR]\label{pfr-torsion}\lean{torsion_PFR}\leanok  Suppose that $G$ is a finite abelian group of torsion $m$.
  ...
  \begin{proof}\leanok Repeat the proof of \Cref{pfr}, but with \Cref{pfr_aux_torsion} in place of \Cref{pfr_aux}.
  \end{proof}
